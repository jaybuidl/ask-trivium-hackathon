...
export const AnalyzeDisputeInput = z.object({
  ...
  idempotency_key: z
    ...
    .optional()
    .describe(
      'Optional UUID, generated by the caller before its first attempt and resent unchanged to ' +
        'retry a dropped call without paying twice. It cannot be derived from the payment nonce: ' +
        'a retry signs a fresh authorization with a fresh nonce, so a derived key would differ ' +
        'from the one it is supposed to match.',
    ),
})
